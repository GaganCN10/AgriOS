...
export function getFriendlyError(err) {
  ...
  if (msg.includes('network') || msg.includes('econnreset') || msg.includes('fetch failed')) return FRIENDLY_ERRORS.NETWORK_ERROR;
  if (msg.includes('timeout')) return FRIENDLY_ERRORS.TIMEOUT;
  ...
  if (msg.includes('validation') || msg.includes('required') || msg.includes('invalid')) return FRIENDLY_ERRORS.VALIDATION;
  ...
  if (msg.includes('farm') && msg.includes('name')) return FRIENDLY_ERRORS.FARM_NAME_REQUIRED;
  if (msg.includes('boundary') || msg.includes('polygon')) return FRIENDLY_ERRORS.POLYGON_INVALID;
  if (msg.includes('overlap')) return FRIENDLY_ERRORS.POLYGON_OVERLAP;
  if (msg.includes('price') && msg.includes('valid')) return FRIENDLY_ERRORS.PRICE_REQUIRED;
  if (msg.includes('yield') && msg.includes('valid')) return FRIENDLY_ERRORS.YIELD_REQUIRED;
  if (msg.includes('task') && msg.includes('title')) return FRIENDLY_ERRORS.TASK_TITLE_REQUIRED;
  if (msg.includes('inventory') && msg.includes('name')) return FRIENDLY_ERRORS.INVENTORY_NAME_REQUIRED;
  if (msg.includes('equipment') && msg.includes('name')) return FRIENDLY_ERRORS.EQUIPMENT_NAME_REQUIRED;
  if (msg.includes('email') && msg.includes('required')) return FRIENDLY_ERRORS.EMAIL_REQUIRED;
  if (msg.includes('password') && msg.includes('6')) return FRIENDLY_ERRORS.PASSWORD_MIN;
  ...
  if (msg.includes('missing') || msg.includes('required')) return FRIENDLY_ERRORS.VALIDATION;
